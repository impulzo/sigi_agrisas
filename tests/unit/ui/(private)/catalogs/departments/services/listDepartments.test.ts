...
describe("listDepartments", () => {
  it("returns items with dates parsed as Date on success 200", async () => {
    ...
    const result = await listDepartments({ page: 1, pageSize: 20 }, mockFetch);
  ...
  it("includes includeInactive=true in the URL when option is set", async () => {
    ...
    await listDepartments({ page: 1, pageSize: 20, includeInactive: true }, mockFetch);
  ...
  it("throws UnauthenticatedError on 401", async () => {
    ...
    await expect(listDepartments({ page: 1, pageSize: 20 }, mockFetch)).rejects.toBeInstanceOf(UnauthenticatedError);
  });

  it("throws ForbiddenError on 403", async () => {
    const mockFetch = jest.fn().mockRejectedValueOnce(new ForbiddenError("departments:read"));

    await expect(listDepartments({ page: 1, pageSize: 20 }, mockFetch)).rejects.toBeInstanceOf(ForbiddenError);
  });

  it("throws NetworkError on network failure", async () => {
    const mockFetch = jest.fn().mockRejectedValueOnce(new NetworkError());

    await expect(listDepartments({ page: 1, pageSize: 20 }, mockFetch)).rejects.toBeInstanceOf(NetworkError);
  });
