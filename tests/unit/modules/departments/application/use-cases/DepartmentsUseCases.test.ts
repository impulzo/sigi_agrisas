...
function makeDept(id: string, code: string, isActive = true): Department {
  const now = new Date();
  return Department.create(id, { code, name: `Dept ${code}`, description: null, isActive, createdAt: now, updatedAt: now });
}
...
describe("CreateDepartmentUseCase", () => {
  it("creates with minimal data", async () => {
    const result = await new CreateDepartmentUseCase(new InMemoryDepartmentRepository()).execute({ code: "SALES", name: "Ventas" });
  ...
  it("throws DepartmentCodeAlreadyInUseError on duplicate", async () => {
    const repo = new InMemoryDepartmentRepository();
    await new CreateDepartmentUseCase(repo).execute({ code: "SALES", name: "Ventas" });
    await expect(new CreateDepartmentUseCase(repo).execute({ code: "SALES", name: "Otro" })).rejects.toThrow(DepartmentCodeAlreadyInUseError);
  });
});
...
describe("UpdateDepartmentUseCase", () => {
  it("updates name", async () => {
    const repo = new InMemoryDepartmentRepository();
    const created = await new CreateDepartmentUseCase(repo).execute({ code: "SALES", name: "Ventas" });
    const result = await new UpdateDepartmentUseCase(repo).execute({ id: created.id, name: "Ventas Corp." });
  ...
  it("clears description with null", async () => {
    const repo = new InMemoryDepartmentRepository();
    const created = await new CreateDepartmentUseCase(repo).execute({ code: "OPS", name: "Ops", description: "Desc" });
    const result = await new UpdateDepartmentUseCase(repo).execute({ id: created.id, description: null });
    expect(result.description).toBeNull();
  });

  it("throws DepartmentNotFoundError for unknown id", async () => {
    await expect(new UpdateDepartmentUseCase(new InMemoryDepartmentRepository()).execute({ id: "x", name: "N" })).rejects.toThrow(DepartmentNotFoundError);
  });
});
...
describe("SoftDeleteDepartmentUseCase", () => {
  ...
  it("is idempotent", async () => {
    const repo = new InMemoryDepartmentRepository();
    const created = await new CreateDepartmentUseCase(repo).execute({ code: "SALES", name: "Ventas", isActive: false });
