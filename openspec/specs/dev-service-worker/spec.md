# Spec: dev-service-worker

## Purpose

Define el registro condicional del Service Worker (`public/sw.js`) en `ServiceWorkerRegistrar`: sólo se registra en producción, nunca en desarrollo o test, para evitar que el navegador quede sirviendo chunks de webpack cacheados y obsoletos tras un reinicio de `next dev`.
