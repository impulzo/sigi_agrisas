...
Pantalla de gestión de proveedores bajo `/catalogs/providers` con datos fiscales mexicanos (RFC, régimen, CFDI), búsqueda server-side debounced, modal con secciones agrupadas (Datos básicos / Datos fiscales / Contacto), soft delete con confirmación y reactivación directa. Conectada al CRUD existente en `/api/v1/admin/providers` y accesible desde el submenú "Catálogos" y el hub `/catalogs`.
...
The system SHALL expose a hook `useProviderMutations()` returning `{ isSaving, mutationError, clearError, createOne, updateOne, softDeleteOne, reactivateOne }`. `reactivateOne(id)` SHALL be a convenience wrapper around `updateProvider(id, { isActive: true })`. `updateOne(id, body)` SHALL NOT dispatch the request when the body is empty; instead it MUST return `null`. The hook SHALL re-throw typed errors (`ProviderCodeAlreadyInUseError`, `ProviderRfcAlreadyInUseError`) so that `ProviderEditModal` can map them to inline field errors; other errors SHALL be captured in `mutationError` and the mutation methods SHALL return `null`.

#### Scenario: createOne success returns the entity
- **WHEN** `createOne(body)` is invoked and the backend returns 201
- **THEN** the method resolves with the created `Provider` and `mutationError` is `null`

#### Scenario: createOne 409 code re-throws ProviderCodeAlreadyInUseError
- **WHEN** `createOne(body)` is invoked and the backend returns 409 with `code already in use`
- **THEN** the method rejects with `ProviderCodeAlreadyInUseError` (caught by the modal)

#### Scenario: createOne 409 rfc re-throws ProviderRfcAlreadyInUseError
- **WHEN** `createOne(body)` is invoked and the backend returns 409 with `RFC already in use`
- **THEN** the method rejects with `ProviderRfcAlreadyInUseError` (caught by the modal)
