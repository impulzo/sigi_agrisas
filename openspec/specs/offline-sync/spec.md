...
Modo offline transversal del panel: instala como PWA, cachea catálogo/inventario de lectura en IndexedDB por `ownerBranchId`, encola ventas y cotizaciones creadas sin conexión, y las sincroniza automáticamente al reconectar vía un único tab líder. Consumido por `pos-ui` y `quotes-ui` (encolado en creación, banners de estado) y respaldado por `pos-api`/`quotes-api` (idempotencia por `clientRequestId`).
