# inventory-seed-data Specification

## Purpose
TBD - created by archiving change sat-unit-code-inventory-seed. Update Purpose after archive.
## Requirements
### Requirement: Mapeo de unidad cruda del Excel a clave SAT
El generador de datos de inventario (`prisma/seeds/data/generate-inventario-tiendas-data.ts`) SHALL traducir el valor crudo de la columna "Unidad" de `INVENTARIOS TIENDAS.xlsx` a una clave real del catálogo SAT `c_ClaveUnidad` mediante una whitelist explícita (`UNIT_CODE_MAP`), antes de emitirlo como `unit` en cada fila de `inventario-tiendas-v3.ts`. El valor `"PZA"` SHALL mapear a `"H87"` (Pieza). El valor `"NA"` SHALL mapear a `"ACT"` (Actividad).

#### Scenario: Fila con Unidad "PZA" mapea a clave Pieza
- **WHEN** el generador procesa una fila del Excel con `Unidad = "PZA"`
- **THEN** la fila emitida en `inventario-tiendas-v3.ts` tiene `unit: "H87"`
...
- **THEN** la fila emitida en `inventario-tiendas-v3.ts` tiene `unit: "ACT"`
...
Al regenerar `inventario-tiendas-v3.ts` con el mapeo de unidad aplicado, el generador SHALL preservar sin cambios todos los demás campos de cada fila y los contadores de ejecución (productos, departamentos, filas de sección, omitidos, colisiones) SHALL ser idénticos a una regeneración sin el mapeo de unidad, para el mismo Excel fuente.

#### Scenario: Regeneración produce mismos conteos que el run previo
- **WHEN** se regenera `inventario-tiendas-v3.ts` contra el mismo `INVENTARIOS TIENDAS.xlsx`
- **THEN** el log del comando reporta los mismos conteos por hoja (productos, departamentos, omitidos, colisiones) que el run anterior al cambio
...
El seeder runtime (`prisma/seeds/lib/inventoryTiendasSeedLogic.ts`) SHALL incluir `unit` tanto en la rama `create` como en la rama `update` de cada `product.upsert` por `code`, de forma que correr `npm run seed:inventory-tiendas` después de regenerar los datos actualice el campo `unit` de productos ya existentes en base de datos sin crear duplicados.
...
- **AND** se corre `npm run seed:inventory-tiendas` con los datos regenerados (`unit: "H87"` para ese `code`)
