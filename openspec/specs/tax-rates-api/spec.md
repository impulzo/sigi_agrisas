...
Define the Tax Rates API: a CRUD catalog of tax rates (`TaxRate`) that products can reference via `taxRateId` for classification and reporting purposes. The catalog centralizes the tax percentage plus its SAT (Servicio de Administración Tributaria) classification fields and accounting accounts, so operators don't hand-type these values on every product. `taxRateId` is informative — sale/quote total calculations keep using the product's own `ivaRate`/`iepsRate`.
...
The system SHALL expose `POST /api/v1/admin/tax-rates`. Requires `tax_rates:write`. Body: `{ code: string (^[A-Z0-9_]{1,32}$), name: string (1-100 chars), description?: string|null, satTaxCode: string (^\d{3}$), factorType: "Tasa" | "Cuota" | "Exento", displayValue: number, rate: number (>= 0, 6 decimal precision), transferredAccount?: string|null (max 20 chars), pendingTransferredAccount?: string|null (max 20 chars), creditedAccount?: string|null (max 20 chars), pendingCreditedAccount?: string|null (max 20 chars), isActive?: boolean (default true) }`. `code` is normalized to uppercase and trimmed. Duplicate `code` returns HTTP 409. When `factorType` is `"Tasa"` or `"Exento"`, `rate` MUST be `<= 1` (HTTP 400 otherwise); `"Cuota"` represents a fixed monetary amount and has no upper bound. Returns HTTP 201 with `TaxRateDto`.
...
#### Scenario: Rate exceeds bound for Tasa/Exento
- **WHEN** `factorType: "Tasa"` and `rate: 1.5`
- **THEN** system returns HTTP 400 with validation error on `rate`

#### Scenario: Cuota rate may exceed 1
- **WHEN** `factorType: "Cuota"` and `rate: 3.50`
- **THEN** system returns HTTP 201 (no upper bound applied)
