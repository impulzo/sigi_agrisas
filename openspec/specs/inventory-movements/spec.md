...
Ledger append-only de movimientos de inventario (`inventory_movements`), alimentado transaccionalmente por cada punto donde el sistema muta `branch_inventory.quantity` (venta, cancelación/edición de venta, devolución, cancelación de devolución, ajuste manual, alta con existencia inicial). Provee la trazabilidad de origen para el reporte de kardex (`inventory-kardex-api`). Sin superficie HTTP propia — es infraestructura de escritura consumida internamente por `pos`, `returns` e `inventory`.
