...
El recibo SHALL incluir el logo del negocio en el encabezado (tamaño normal, al ser un documento entregado al cliente), resuelto desde `TicketSettings.logoUrl` con fallback al logo por defecto. Los colores del recibo SHALL provenir de la paleta de marca compartida (`pdfTheme`).
...
El header del PDF (ambos endpoints: resumen y libro mayor) SHALL incluir el logo del negocio (tamaño reducido), resuelto desde `TicketSettings.logoUrl` con fallback al logo por defecto. Los colores de tabla SHALL provenir de la paleta de marca compartida (`pdfTheme`).
