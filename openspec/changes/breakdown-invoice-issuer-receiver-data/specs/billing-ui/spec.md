...
The private route `/billing/csd` (gated by `billing:manage_csd`) SHALL render a "Dirección" field (free text, optional) in the CSD upload form, alongside `rfc`, razón social, régimen fiscal, and CP. It SHALL follow the same prefill-on-load and partial-upsert semantics already documented for those fields: prefilled from `GET /api/v1/admin/billing/csd` on load, and an empty submission leaves the previously stored value unchanged.
