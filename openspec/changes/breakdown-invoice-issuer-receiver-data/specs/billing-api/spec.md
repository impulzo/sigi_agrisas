...
- **THEN** the system returns HTTP 200 with the 5 fields, resolved via the cascade (CSD status → `EmitterFiscalSettings` → fixed test data)

#### Scenario: Nothing captured yet — fixed test data returned
- **WHEN** there is no CSD loaded in the Facturama account and `EmitterFiscalSettings` has no row (or all fields `null`)
- **THEN** the system returns HTTP 200 with the fixed test-data values (never `null`, never a dash placeholder)
...
The system SHALL resolve the issuer's fiscal identity (`rfc`, `legalName`, `fiscalRegime`, `zipCode`, `address`) via a single shared, pure resolution function consumed by both "Read emitter fiscal settings (lightweight)" and "Persist invoice with fiscal snapshot", using this cascade, per field:
...
2. **`EmitterFiscalSettings`** (local) — used for any field tier 1 did not resolve (which, per the above, is always `fiscalRegime`/`zipCode`/`address`, and `rfc`/`legalName` when tier 1 failed or had no CSD loaded).
3. **Fixed test data** — a hardcoded fallback (same values as `FakeFacturamaGateway`'s `FALLBACK_INVOICE_DATA`, extended with a test `address`) used for any field still unresolved after tiers 1 and 2.
...
- **THEN** the resolved `rfc`/`legalName` SHALL equal those values, while `fiscalRegime`/`zipCode`/`address` SHALL still come from `EmitterFiscalSettings` (or test data if that is also empty)

#### Scenario: No CSD loaded — falls through to EmitterFiscalSettings
- **WHEN** `getCsdStatus()` fails or returns no usable `rfc`/`issuer`
- **THEN** `rfc`/`legalName` SHALL come from `EmitterFiscalSettings`, or from fixed test data if that row is also empty for those fields

#### Scenario: Nothing resolvable anywhere — fixed test data used
- **WHEN** tiers 1 and 2 both fail to resolve a given field
- **THEN** that field SHALL take the fixed test-data value, never `null`
...
The issuer snapshot (`issuerRfc`/`issuerLegalName`/`issuerFiscalRegime`/`issuerZipCode`/`issuerAddress`) SHALL be resolved server-side, inside the stamping use case (both "stamp from sale" and "stamp standalone" flows), using the cascade described in "Resolve issuer fiscal data (cascade)" — NEVER from a client-supplied value in the stamp request body. Because the cascade's final tier is fixed test data (never `null`), an invoice stamped after this capability exists SHALL always have all 5 `issuer*` columns populated with a non-empty value — `null` SHALL only ever appear on invoices stamped before this capability existed.
...
- **THEN** the header SHALL show the logo and the title "Factura" (not a company name), followed by an "Emisor" section with RFC, legal name, fiscal regime (code + description), zip code, and address — none of them blank, thanks to the cascade's fixed-test-data final tier
