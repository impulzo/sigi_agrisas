...
El toolbar incluye:
- Botón "Exportar PDF" que: (1) llama `GET /api/v1/admin/payments/history?format=pdf&{filtros actuales}` con `authFetch`; (2) convierte la respuesta a `Blob` y crea un `<a download="payments-history-YYYY-MM-DD.pdf">` dinámico; (3) muestra spinner durante la descarga; (4) error 409 (`ReportTooLarge`) → toast "El conjunto de datos supera 10,000 registros. Aplica más filtros." El PDF generado agrupa las filas por ticket.
- Botón "Exportar Excel" con el mismo comportamiento que "Exportar PDF" pero llamando `?format=xlsx` y descargando `payments-history-YYYY-MM-DD.xlsx`. El Excel generado agrupa las filas por ticket con la misma estructura que el PDF.
