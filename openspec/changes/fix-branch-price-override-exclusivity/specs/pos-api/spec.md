...
- **WHEN** the body's `branchId` is HUAJUAPAN and an item's `productPriceId` references a `ProductPrice` whose `branchId = null` (base), and the product has NO override for HUAJUAPAN
