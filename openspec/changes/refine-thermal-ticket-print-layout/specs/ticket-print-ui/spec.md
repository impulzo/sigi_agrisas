...
- **THEN** the `img` is sized to 125px wide × 77px tall with `object-fit: contain` so the aspect ratio is preserved (no distortion) — corrige el valor previo "75x105px", inconsistente con el código real y con el Requirement "Proporción y margen del logo consistentes entre preview e impresión"

#### Scenario: Browser print uses page margins
- **WHEN** the ticket prints via `window.print()` (browser mode)
- **THEN** the CSS `@page` rule declares `margin: <margenTop>mm 3mm 4mm 3mm` (margen superior = 4mm + 5% del alto de contenido, laterales 3mm, inferior 4mm); the `.printable-ticket` container also declares `position: absolute !important; top: 0 !important; left: 0 !important` as reinforcement against thermal printer drivers, which does not conflict with the `@page` margin — verified by generating an actual PDF (`page.pdf({ preferCSSPageSize: true })`) with and without the forcing and confirming the rendered output is byte-identical
...
El contenido imprimible del ticket SHALL usar `position: absolute !important; top: 0 !important; left: 0 !important` en `.printable-ticket` para desacoplarse del flujo de layout de la vista previa y reforzar el anclaje contra drivers de impresoras térmicas (ej. EPSON TM-T20II) que pudieran sustituir el layout normal por uno propio. Esta combinación fue introducida por `document-thermal-print-limitation` y removida por `add-ticket-print-margins` (que asumió, sin verificación empírica, que anulaba el margen de `@page`). Este change resuelve formalmente el conflicto: al generar el PDF real de impresión (`page.pdf({ preferCSSPageSize: true })`, no solo emular `media: print` en el DOM normal — `getBoundingClientRect()` bajo `emulateMedia` no refleja el efecto de `@page margin`, que solo se aplica en el pipeline de paginación real), el output con y sin el forcing resultó byte-idéntico. Es decir, un elemento `position: absolute; top: 0; left: 0` se ancla al origen de su contenedor de bloque inicial, que en el pipeline de impresión de Chromium ya es el área de contenido posterior al margen de página — nunca compitió con `@page margin`. Por lo tanto el forcing se reinstala de forma permanente como refuerzo adicional, sin riesgo de anular los márgenes.

#### Scenario: Anclaje respeta el margen de @page en la impresión real
- **WHEN** el cajero ejecuta `window.print()` desde la vista previa, o se genera el PDF de impresión
- **THEN** el contenido de `.printable-ticket` aparece anclado dentro del margen declarado por `@page` (no en la esquina física del papel, no centrado) — verificado generando el PDF real con Playwright (`page.pdf({ preferCSSPageSize: true })`) y confirmando visualmente el margen superior/lateral

#### Scenario: Resolución formal del conflicto entre forcing de anclaje y margen de @page
- **WHEN** se evalúa si `top: 0 !important; left: 0 !important` en `.printable-ticket` interfiere con el margen de `@page` (4mm base + margen superior adicional, 3mm laterales, 4mm inferior)
- **THEN** no interfiere — verificado comparando byte a byte el PDF generado con y sin el forcing (idénticos); se reinstala el forcing de forma permanente como refuerzo adicional contra drivers de impresoras térmicas, reemplazando los escenarios de "fuera de alcance"/"limitación de driver" declarados por los changes previos para este mismo conflicto
...
- **THEN** `.printable-ticket` y sus hijos (`.printable-ticket *`) usan `box-sizing: border-box`, reduciendo la probabilidad de que el driver reescale el contenido por desborde de ancho — este refuerzo de ancho es independiente del anclaje y no se ve afectado por la resolución del conflicto de `top`/`left`

#### Scenario: Verificación final pendiente de confirmación en hardware real
- **WHEN** este ajuste (resolución de anclaje + margen superior adicional + colchón de altura recalibrado) se despliega sin una impresora física TM-T20II disponible en el entorno de desarrollo
- **THEN** el cierre de este change queda condicionado a que el cliente confirme en la impresora física que el ticket sale anclado correctamente, sin corte de contenido y sin hoja en blanco sobrante; si persiste alguna limitación de driver/SO, se documenta como tal sin bloquear otras funcionalidades del ticket
