...
- [x] 8.3 Verificación manual en navegador completada vía Playwright (la extensión Chrome seguía fallando por timeout de inyección de script; se detectó además que el dev server previo corría bajo Node 18.0.0 en vez de la versión de `.nvmrc` (24.14.1), causando chunk-loading errors — se reinició con la versión correcta). Flujo cubierto end-to-end como admin: login → `/purchases` (toolbar, filtros, empty state, item de nav activo) → `/purchases/new` (ProviderPicker con búsqueda + quick-add, Sucursal condicionada a `branches:access_all`, Forma de pago con aviso de crédito, ProductPicker, línea agregada) → submit → redirect a `/purchases/[id]` (folio `CP-000001`, badge, items table, meta panel, `ProviderPaymentsSection` con CTA condicionado, `PurchaseActionsBar`) → modal "Registrar abono" (saldo pendiente) → modal "Cancelar compra" (motivo obligatorio, submit deshabilitado sin razón) → cancelación de compra via API (revierte inventario, folio no se libera, `paymentStatus` se mantiene consistente). Cero errores de consola/pageerror/5xx en todo el flujo. Datos de prueba (proveedor + compra) limpiados al final (compra cancelada, proveedor desactivado).
