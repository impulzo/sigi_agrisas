...
## ADDED Requirements — Reporte de Historial de Abonos
...
Idéntico al branch scoping del reporte de inventario. `resolveScopedBranchId(req, parsed.branchId)` resuelve el branchId efectivo; sin `branches:access_all`, el usuario solo consulta su propia sucursal. El `branchId` efectivo filtra los abonos vía `customer_payments.sale.branch_id`.
