...
#### Scenario: Tax regime printed in full when configured via the SAT catalog
- **WHEN** `GET /settings/ticket` returns `businessTaxRegime: "612 — Personas Físicas con Actividad Empresarial"` (captured via the SAT catalog combobox in `settings-ui`)
- **THEN** the printed ticket shows that full string as-is — never just the leading code, never truncated

#### Scenario: Legacy free-text tax regime still prints
- **WHEN** `GET /settings/ticket` returns a `businessTaxRegime` value saved before this change (e.g. `"612"` with no description)
- **THEN** the printed ticket shows that value unchanged — no error, no forced re-formatting
...
#### Scenario: Logo rendered at 105x147px
- **WHEN** the printed ticket renders its logo (either `logoUrl` from settings or the `/logo.png` fallback)
- **THEN** the `img` is sized to 105px wide × 147px tall (a 40% increase over the prior 75×105px) with `object-fit: contain` so the aspect ratio is preserved (no distortion), and the container's width is unaffected by `paperWidth`

#### Scenario: Logo bottom margin unchanged by the size increase
- **WHEN** the printed ticket renders its enlarged logo above the business info section
- **THEN** the bottom margin separating the logo from the content below it remains `2.4px` — the size increase does not shift any subsequent section
