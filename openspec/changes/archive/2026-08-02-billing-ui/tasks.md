...
- [x] 10.2 Recorrido manual con `FACTURAMA_MOCK=true`: facturar venta desde `/sales/[id]` → 201, detalle muestra estado "Vigente"; descarga PDF → 200; descarga XML → 200; cancelar CFDI con motivo → estado "Cancelada" con motivo/fecha; `/billing/new` tab "Factura parcial" renderiza correctamente (banner "no afecta inventario", conceptos, totales, submit disabled sin líneas); `/billing/csd` GET status muestra CSD mock activo y formulario de carga presente. Ejecutado contra BD real (única disponible) por decisión del usuario.
