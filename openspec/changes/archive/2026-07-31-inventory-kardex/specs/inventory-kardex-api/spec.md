...
El sistema SHALL aplicar `resolveScopedBranchId(req, branchId, authz)` en la consulta y `enforceBranchScope` en la reconstrucción. Sin `branches:access_all`, `branchId` SHALL forzarse a la sucursal del usuario aunque la request pida otra o ninguna ("todas"). Con `branchId` omitido y bypass, el header SHALL agregar todas las sucursales en scope. (Traza: S2/S4 Seguridad.)
