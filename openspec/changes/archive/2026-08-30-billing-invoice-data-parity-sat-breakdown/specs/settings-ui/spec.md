...
- **THEN** the page shows the current configuration, including razón social, RFC, and código postal, but all inputs/upload/delete controls are disabled

#### Scenario: Editable view with settings:write
- **WHEN** a user has `settings:write`
- **THEN** the page allows uploading/deleting the logo, editing razón social, RFC, business address/phone/tax regime/zip code, footer text, and changing the paper width, persisting changes via `settings-api`
...
#### Scenario: Zip code field saves and reloads
- **WHEN** a user with `settings:write` enters `"83000"` in the código postal field and saves
- **THEN** the request sends `{ "businessZipCode": "83000" }`, and reopening `/settings` shows the field pre-filled with that value

#### Scenario: Zip code field validates 5-digit format client-side
- **WHEN** a user enters a value that is not 5 digits in the código postal field and attempts to save
- **THEN** the form SHALL show a validation error and SHALL NOT submit the request, mirroring the backend's `^\d{5}$` constraint
