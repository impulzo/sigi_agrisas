...
no tiene delta de UI aquí porque el comportamiento de "Deactivate with products" (arriba, sin
cambios) ya cubre el escenario de bloqueo — no se modifica en este change.

"Tax rates card in catalogs hub" no aparece en este delta: el requirement del change `tax-rates`
queda sin cambios (mismo texto), así que no corresponde declararlo MODIFIED aquí.
-->
