...
- [x] 2.3 Agregar `tax_rates:read` y `tax_rates:write` al array de permisos en `prisma/seed.ts` (no existe `prisma/seeds/rbac.ts` separado); asignar `tax_rates:read` a `viewer`, `admin`, `operator`; `tax_rates:write` a `admin`, `operator`.
