...
<!--
Nota de trazabilidad: el change `tax-rates-sat-fields` (openspec/changes/tax-rates-sat-fields/)
ya fue implementado sobre este módulo y su delta MODIFICA los requirements "List tax rates",
"Get tax rate detail", "Create tax rate" y "Update tax rate" de abajo, agregando `satTaxCode`,
`factorType`, `displayValue` y 4 cuentas contables como campos obligatorios/adicionales del
`TaxRateDto` y del body de create/update. Los ejemplos de body/scenario de "Create tax rate" y
"Update tax rate" aquí abajo reflejan el alcance ORIGINAL de este change (pre-sat-fields) y ya
no producen el resultado documentado contra el código actual — el body mínimo `{ code, name,
rate }` ahora responde 400 por falta de `satTaxCode`/`factorType`/`displayValue`. Ver el delta
de `tax-rates-sat-fields` para el comportamiento vigente. Se recomienda archivar ambos changes
juntos para que la sync a `openspec/specs/tax-rates-api/spec.md` aplique los dos deltas en orden.
-->
