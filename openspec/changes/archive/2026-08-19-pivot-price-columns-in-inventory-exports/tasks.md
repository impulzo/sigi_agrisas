...
- [x] 5.3 Manual: descargar PDF y Excel de `/reports/inventory` en ambos tabs (Por Departamento y Global) y confirmar que las columnas de precio coinciden con lo que ya muestra `InventoryPriceStockTable.tsx` en pantalla. — **bloqueado de nuevo** (opsx:verify, 2026-08-19): mismo fallo persistente de la extensión de Chrome. Sustituido: `priceColumnNames.ts` (fuente compartida) confirmado usado tanto por `InventoryPriceStockTable.tsx` como por `DepartmentPriceListReportPdf.tsx`/`buildDepartmentPriceListWorkbook.ts`, más el test de contrato XLSX en `ReportsController.test.ts` (header dinámico, fila-por-producto, "—" si falta lista) — 97/97 en verde.
