...
El sistema SHALL exponer `GET /api/v1/admin/sat-codes?search=` que requiere sesión autenticada (sin permiso adicional — catálogo de referencia, no dato de negocio sensible). Query param `search` (opcional, mínimo 2 caracteres cuando presente) filtra por `code` O `description` vía `ILIKE '%search%'`. Sin `search`, devuelve una página inicial (máx. 20 resultados) para no listar el catálogo completo (52,513 códigos) sin filtro. Response: `{ items: Array<{code: string, description: string}> }`, sin paginación explícita — el tope de 20 resultados es intencional y suficiente para el autocompletado del combobox de productos; el filtro `search` es el mecanismo para navegar el catálogo completo.
...
El seed SHALL sembrar un subconjunto curado de códigos SAT relevantes al giro agro/agroquímico del cliente (~50-100 filas) y SHALL incluir un comentario visible marcando los datos como placeholder no verificado. Reemplazado por el catálogo real completo (`Requirement: Seed del catálogo real c_ClaveProdServ`).
