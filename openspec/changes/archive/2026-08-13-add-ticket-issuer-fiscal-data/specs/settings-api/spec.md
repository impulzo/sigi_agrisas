...
- **THEN** the singleton row is created (if absent) or updated (if present) with the real `businessName`, `businessRfc`, `businessAddress`, `businessPhone`, `businessTaxRegime` — other fields (`logoUrl`, `headerText`, `footerText`, `paperWidth`, `legendText`) are left untouched if the row already existed

#### Scenario: Configuration exists
- **WHEN** a row exists with `logo_url`, `header_text`, `footer_text`, `paper_width`, `business_name`, `business_rfc`, `business_address`, `business_phone`, `business_tax_regime`, `legend_text` set
