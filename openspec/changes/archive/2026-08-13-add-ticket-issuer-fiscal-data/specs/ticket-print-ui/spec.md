...
- **THEN** the printed ticket shows the business section under the header, in this order: razón social (`businessName`), "RFC: <businessRfc>", address, "Tel. <phone>", tax regime; when a field is `null`, that specific line is omitted without breaking the layout
...
- **THEN** the print view still renders with the sale data, omitting logo/header/footer/business (name, RFC, address, phone, tax regime)/legend (falling back to `80mm` width) rather than blocking the print action entirely
