...
- **THEN** the rail SHALL render the `quotes` item (label "Cotizaciones", href `/quotes`, icon `request_quote`) between `sales` and `inventory`
...
- **THEN** the rail SHALL render the `inventory` item (label "Inventario", href `/inventory`) between `quotes` and `catalogs`
