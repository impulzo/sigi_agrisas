...
### NavigationRail
Nuevo `RailItem { key: "billing", href: "/billing", icon: "description", label: "Facturación", requires: "billing:read" }` (icon distinto de `sales`'s `receipt_long` para evitar duplicado visual en el rail), ubicado tras `returns` (en la práctica, tras `payments` por orden de inserción posterior de ese módulo). CSD no es item propio del rail; se accede desde un botón dentro de `/billing` (visible con `manage_csd`).
