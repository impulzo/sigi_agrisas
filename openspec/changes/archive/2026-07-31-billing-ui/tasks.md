...
- [x] 8.2 `NavigationRail` — agregar `RailItem { key:"billing", href:"/billing", icon:"receipt_long", label:"Facturación", requires:"billing:read" }` tras `returns`.
...
- [x] 10.2 Recorrido manual con `FACTURAMA_MOCK=true`: facturar venta, factura parcial (verificar inventario intacto), cancelar, descargar PDF/XML, cargar CSD.
