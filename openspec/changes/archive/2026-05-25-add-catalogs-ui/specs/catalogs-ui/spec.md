...
The system SHALL expose a private route `/catalogs` that serves as a hub for the four catalog modules. The hub SHALL render a grid of four entry cards, one per catalog (`payment-methods`, `folios`, `departments`, `branches`). Each card SHALL display the catalog icon, title, a short description, and an "Abrir" link to `/catalogs/<módulo>`. Cards SHALL be visually marked as disabled when the current user lacks the corresponding `<recurso>:read` permission, and SHALL render normally otherwise. The hub itself SHALL be reachable by any authenticated user.

#### Scenario: Authenticated user opens the hub
- **WHEN** an authenticated user navigates to `/catalogs`
- **THEN** the page SHALL render four entry cards (Formas de pago, Folios, Departamentos, Sucursales) with their icons and short descriptions
...
- **THEN** the router SHALL navigate to `/catalogs/<módulo>` (e.g., `/catalogs/payment-methods`)
