...
- [x] 7.1 `npm test` completo — 0 regresiones fuera de lo ya conocido como pre-existente (documentar cualquier fallo pre-existente igual que en el change anterior, aislando con `git stash`).
...
- [ ] 7.3 Con `EmitterFiscalSettings` vacío (estado real de este entorno) y el CP recién guardado en Ticket de venta, timbrar una factura de prueba y confirmar `issuerZipCode` en la respuesta.
- [x] 7.4 Abrir el detalle web de esa factura y descargar su PDF — comparar campo por campo (emisor incluyendo `issuerBranchName`, receptor, forma/método de pago, cada línea con clave SAT+descripción/IEPS/subtotal) y confirmar que coinciden exactamente.
- [ ] 7.5 Abrir una prefactura (modal) para una cotización o venta con líneas que tengan `satProductCode` e IEPS>0, descargar su PDF de borrador, y comparar modal vs. PDF borrador campo por campo.
