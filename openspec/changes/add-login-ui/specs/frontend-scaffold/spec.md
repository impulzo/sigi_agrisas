...
El proyecto SHALL tener Tailwind CSS v3 instalado y configurado para funcionar con Next.js 14 App Router, con `content` apuntando a `./app/**/*.{ts,tsx}`.
...
#### Scenario: Variables de color del diseño disponibles como tokens Tailwind
- **WHEN** un desarrollador usa `text-agrisas-dark` o `bg-agrisas-mint` en un componente
- **THEN** Tailwind aplica los valores de la paleta definida en `tailwind.config.ts` (#1a4d42, #2a6b5f, #d4f1e9, #e8f7f3)

#### Scenario: Fuentes Inter y Poppins disponibles como utilidades
- **WHEN** un componente usa `font-inter` o `font-poppins`
- **THEN** Tailwind aplica la familia de fuente cargada vía `next/font/google` y mapeada en `theme.extend.fontFamily`
