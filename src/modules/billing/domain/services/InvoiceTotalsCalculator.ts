export interface InvoiceLineInput {
  quantity: number;
  unitPrice: number;
  discountPct?: number | null;
  ivaRate?: number | null;
  iepsRate?: number | null;
}

export interface InvoiceLineTotals {
  lineSubtotal: number;
  lineIva: number;
  lineIeps: number;
  lineTotal: number;
  taxObject: string;
}

export interface InvoiceTotalsResult {
  lines: InvoiceLineTotals[];
  subtotal: number;
  taxTotal: number;
  total: number;
}

// Half-to-even (banker's) rounding — same algorithm as SaleTotalsCalculator
function roundHalfToEven(value: number, decimals: number): number {
  const factor = Math.pow(10, decimals);
  const scaled = value * factor;
  const floor = Math.floor(scaled);
  const diff = scaled - floor;
  const eps = 1e-9;
  let rounded: number;
  if (diff > 0.5 + eps) {
    rounded = floor + 1;
  } else if (diff < 0.5 - eps) {
    rounded = floor;
  } else {
    rounded = floor % 2 === 0 ? floor : floor + 1;
  }
  return rounded / factor;
}

const SCALE = 4;

export class InvoiceTotalsCalculator {
  static computeTotals(lines: InvoiceLineInput[]): InvoiceTotalsResult {
    const lineTotals: InvoiceLineTotals[] = [];
    let subtotal = 0;
    let taxTotal = 0;
    let total = 0;

    for (const line of lines) {
      if (!Number.isFinite(line.quantity) || line.quantity <= 0) {
        throw new Error("quantity must be > 0");
      }
      if (!Number.isFinite(line.unitPrice) || line.unitPrice < 0) {
        throw new Error("unitPrice must be >= 0");
      }
      const discountPct = line.discountPct ?? 0;
      if (!Number.isFinite(discountPct) || discountPct < 0 || discountPct > 100) {
        throw new Error("discountPct must be between 0 and 100");
      }
      const ivaRate = line.ivaRate ?? 0;
      const iepsRate = line.iepsRate ?? 0;
      if (!Number.isFinite(ivaRate) || ivaRate < 0 || ivaRate > 1) {
        throw new Error("ivaRate must be between 0 and 1");
      }
      if (!Number.isFinite(iepsRate) || iepsRate < 0 || iepsRate > 1) {
        throw new Error("iepsRate must be between 0 and 1");
      }

      const lineSubtotal = roundHalfToEven(
        line.quantity * line.unitPrice * (1 - discountPct / 100),
        SCALE
      );
      const lineIva = roundHalfToEven(lineSubtotal * ivaRate, SCALE);
      const lineIeps = roundHalfToEven(lineSubtotal * iepsRate, SCALE);
      const lineTotal = roundHalfToEven(lineSubtotal + lineIva + lineIeps, SCALE);
      // taxObject: '02' if any tax applies, '01' if no taxes
      const taxObject = ivaRate > 0 || iepsRate > 0 ? "02" : "01";

      lineTotals.push({ lineSubtotal, lineIva, lineIeps, lineTotal, taxObject });
      subtotal = roundHalfToEven(subtotal + lineSubtotal, SCALE);
      taxTotal = roundHalfToEven(taxTotal + lineIva + lineIeps, SCALE);
      total = roundHalfToEven(total + lineTotal, SCALE);
    }

    return { lines: lineTotals, subtotal, taxTotal, total };
  }
}
