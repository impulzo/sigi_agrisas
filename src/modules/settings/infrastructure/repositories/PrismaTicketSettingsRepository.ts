...
export class PrismaTicketSettingsRepository implements TicketSettingsRepository {
  ...
  async update(data: UpdateTicketSettingsData): Promise<TicketSettings> {
    const row = await this.prisma.ticketSettings.upsert({
      ...
      create: {
        id: SINGLETON_ID,
        headerText: data.headerText ?? DEFAULT_TICKET_SETTINGS.headerText,
        footerText: data.footerText ?? DEFAULT_TICKET_SETTINGS.footerText,
        paperWidth: data.paperWidth ?? DEFAULT_TICKET_SETTINGS.paperWidth,
        businessAddress: data.businessAddress ?? DEFAULT_TICKET_SETTINGS.businessAddress,
        businessPhone: data.businessPhone ?? DEFAULT_TICKET_SETTINGS.businessPhone,
        businessTaxRegime: data.businessTaxRegime ?? DEFAULT_TICKET_SETTINGS.businessTaxRegime,
        legendText: data.legendText ?? DEFAULT_TICKET_SETTINGS.legendText,
      },
      update: {
