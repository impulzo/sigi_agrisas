...
export function PurchasesListPage() {
  ...
  const { items, total, isLoading, error } = usePurchasesList({
    page,
    pageSize,
    status: statusFilter,
  ...
  if (canRead === false) {
    return (
      <EmptyState
        icon="block"
        title="Sin acceso"
        description="No tienes permiso para ver compras."
      />
    );
  }
    ...
    <CatalogShell
      ...
      toolbar={
        <PurchasesToolbar
          ...
          onProviderChange={(v) => { setProviderId(v); setPage(1); }}
          branchId={branchId}
      ...
      {!isLoading && items.length === 0 ? (
        ...
        <>
          <PurchasesTable
            ...
            onEnter={(p) => router.push(`/purchases/${p.id}`)}
          />
          <CatalogPagination
            page={page}
            pageSize={pageSize}
