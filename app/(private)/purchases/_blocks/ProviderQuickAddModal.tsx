...
import { providerQuickAddSchema } from "../_logic/schemas/providerQuickAdd.schema";
import { createProvider } from "../_logic/services/createProvider";
...
interface ProviderQuickAddModalProps {
  onCreated: (provider: ProviderDto) => void;
  onClose: () => void;
}
...
export function ProviderQuickAddModal({ onCreated, onClose }: ProviderQuickAddModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result = providerQuickAddSchema.safeParse(input);
    if (!result.success) {
    ...
    try {
      ...
      if (err instanceof ProviderCodeAlreadyInUseError) {
        ...
        setErrors((prev) => ({ ...prev, rfc: "Este RFC ya está en uso por otro proveedor." }));
      } else {
        setGlobalError("Error al crear el proveedor. Inténtalo de nuevo.");
      }
    } finally {
