...
import { useBranches } from "../_logic/hooks/useBranches";
...
import { BranchCodeAlreadyInUseError } from "../_logic/errors";
...
export function BranchesPage() {
  ...
  const { items, total, isLoading, error, refresh } = useBranches({ page, pageSize, includeInactive });
  const { isSaving, createOne, updateOne, softDeleteOne, reactivateOne, clearError } = useBranchMutations();
  ...
  const handleSave = useCallback(
    async (data: CreateBranchBody | UpdateBranchBody) => {
      if (!modalState) return;
      try {
        if (modalState.mode === "create") {
          const result = await createOne(data as CreateBranchBody);
          if (result !== null) {
            refresh();
            setModalState(null);
          ...
          const result = await updateOne(modalState.entity.id, data as UpdateBranchBody);
          if (result !== null) {
            refresh();
            setModalState(null);
        ...
        if (err instanceof BranchCodeAlreadyInUseError) {
  ...
  const handleSoftDelete = useCallback(
    async (id: string) => {
      ...
      if (ok) {
        ...
        refresh();
        showToast("Sucursal desactivada.");
      }
    },
    ...
    return (
      <div className="flex items-center justify-center h-[60vh]">
        <EmptyState
          ...
          description="No tienes permisos para ver las sucursales. Contacta a un administrador."
        />
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        description="Esta acción marcará la sucursal como inactiva. Podrá reactivarse posteriormente."
