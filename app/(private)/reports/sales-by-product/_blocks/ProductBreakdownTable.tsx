...
import type { SalesByProductRowDto } from "../_logic/types/api";
      ...
      <table className="w-full text-body-sm">
        <thead>
          <tr className="border-b border-outline-variant text-label-sm text-on-surface-variant uppercase tracking-wide">
            ...
            <th className="px-4 py-3 text-right font-medium">Piezas vendidas</th>
          ...
          {rows.length === 0 ? (
            ...
            rows.map((r) => (
              <tr key={r.key} className="border-b border-outline-variant/40">
                ...
                <td className="px-4 py-3 text-right tabular-nums">{r.quantitySold}</td>
                <td className="px-4 py-3 text-right tabular-nums">{r.currentStock}</td>
