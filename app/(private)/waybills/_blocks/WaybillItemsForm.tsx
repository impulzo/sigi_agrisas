...
export function WaybillItemsForm({ lines, addLine, updateLine, removeLine }: WaybillItemsFormProps) {
    ...
    <div>
      ...
      {lines.length > 0 ? (
          ...
          <table className="w-full text-body-sm">
            <thead>
              <tr className="border-b border-outline-variant text-label-sm text-on-surface-variant uppercase tracking-wide">
                ...
                <th className="px-2 py-2 text-left font-medium w-32">Clave SAT</th>
                <th className="px-2 py-2 text-left font-medium w-24">Unidad</th>
                <th className="px-2 py-2 text-right font-medium w-20">Cant.</th>
                <th className="px-2 py-2 text-right font-medium w-24">Peso (kg)</th>
                <th className="px-2 py-2 text-left font-medium w-24">Peligroso</th>
        ...
        <p className="text-body-sm text-on-surface-variant py-4 text-center border border-outline-variant border-dashed rounded-xl">
          Sin mercancías. Agrega del catálogo o como línea libre.
        </p>
      )}
    </div>
  );
