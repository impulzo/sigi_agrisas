...
import type { CancelReturnRequest, ReturnDetailDto } from "../types/api";
...
export async function cancelReturn(id: string, body: CancelReturnRequest, fetchImpl = authFetch): Promise<ReturnDetail> {
  let res: Response;
  try {
    res = await fetchImpl(`/api/v1/admin/returns/${id}/cancel`, {
      method: "POST",
    ...
    if (err instanceof ForbiddenError) {
      ...
      throw new ReturnCancelForbiddenError();
    }
    throw new NetworkError();
  }

  if (res.status === 404) throw new ReturnNotFoundError();
  if (res.status === 409) throw new ReturnAlreadyCancelledError();
